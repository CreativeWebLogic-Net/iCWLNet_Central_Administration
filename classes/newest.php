<?php
	...
	class clsDatabaseInterface{
		...
		var $cls_array=array();
		var $cls_map=array("dc"=>"clsDatabaseConnect","ud"=>"UpdateDatabase","bc"=>"BulkDBChange","ad"=>"AddToDatabase","dd"=>"DeleteFromDatabase");
		...
		function Initialise(){
			...
			$this->$cls_array["dc"]=&$this->m;
			$this->links = $this->m->Connect();

			$this->UpdateDatabase = new UpdateDatabase();
			$this->UpdateDatabase->Set_Log($this->log);
			$this->UpdateDatabase->Set_Vs($this->vs);
			$this->UpdateDatabase->Set_DB($this);
			$this->cls_array["ud"]=&$this->UpdateDatabase;
			$this->BulkDBChange = new BulkDBChange();
			$this->BulkDBChange->Set_Log($this->log);
			$this->BulkDBChange->Set_Vs($this->vs);
			$this->BulkDBChange->Set_DB($this);
			$this->cls_array["bc"]=&$this->BulkDBChange;
			$this->AddToDatabase = new AddToDatabase();
			$this->AddToDatabase->Set_Log($this->log);
			$this->AddToDatabase->Set_Vs($this->vs);
			$this->AddToDatabase->Set_DB($this);
			$this->cls_array["ad"]=&$this->AddToDatabase;
			$this->DeleteFromDatabase = new DeleteFromDatabase();
			$this->DeleteFromDatabase->Set_Log($this->log);
			$this->DeleteFromDatabase->Set_Vs($this->vs);
			$this->DeleteFromDatabase->Set_DB($this);
			$this->cls_array["dd"]=&$this->DeleteFromDatabase;

			$this->r=&$this;
		}
		
		public function Return_Cls($cls_str,$return_txt=false){
			if($return_txt){
				if(str_len($cls_str)<=2){
					$return_str=$this->cls_map[$cls_str];
				}else{
					$return_str=array_search($cls_str, this->cls_map);
				}
				return $return_str;
			}else{
				if(str_len($cls_str)<=2){
					$target_cls=&$this->cls_array[$cls_str];
				}else{
					$target_str=array_search($cls_str, this->cls_array);
					$target_cls=&$this->cls_array[$target_str];
				}
				return $target_cls;
			}
		}
		
		public function Exec($cls_str,$cls_meth,$params=array()){
			$target_cls=&$this->cls_array[$cls_str];
			$ret_val=call_user_func_array('$this->'.$cls_str.'->'.$cls_meth, $params);
			return $ret_val;
			/*
			$this->cls_array[$cls_str]->{$cls_meth}($params);
			if(isset($this->cls_array[$cls_str])){
		...
		*/
		...
		function Set_Database($ser_nam,$dbname){
			...
			$this->ChangeDBFile(1,$dbname);
		}
		
		
		function ChangeDBFile($db_array_num=1,$external_db_name=""){	
			
			$this->log->general('Set New Database: $r->',1);
			$this->r->ChangeDBFile($db_array_num,$external_db_name);
		}
	...
	class BulkDBChange{
		...
		*/
		...
		function __construct(){
			/*
			$this->log = new clsLog();
			$this->log->general("-Add To Db constructor Loaded-",3);
			$this->r = new clsDatabaseInterface();
			$this->r->Set_Log($this->log);
			$this->r->CreateDB();
			$this->log->general("-Add To Db constructor Finished-",3);
			*/
		}
	...
	class clsDatabaseConnect{
		...
		function ChangeDBFile($db_array_num=1,$external_db_name=""){	
			if($external_db_name!=""){
				$this->links[$current_server_tag]->select_db($external_db_name);
				$this->server_login['dbName']=$external_db_name;
			}else{
				$this->log->general('Set New Database: $r->',1);
				$new_db=$this->server_login['dbNames'][$db_array_num];
				$this->links[$current_server_tag]->select_db($new_db);
				$this->server_login['dbName']=$new_db;
			}
		...
		*/
		...
		private function Initialise_Current_Server(){
			...
			switch($current_directory){
				...
				break;
				//-----------------------------------------------------------------------------------------------------------
				case "/homepages/43/d922345856/htdocs/bcms":
					$DB['server_tag']="db-ionos.php";
					$DB['server_desc']="Shared Cheap Server";
					$DB['current_dir']="/homepages/43/d922345856/htdocs/bcms";
					$DB['server_number']=7;
					$DB['hostname']="db5009991482.hosting-data.io";
					$DB['usernamedb']="dbu1555205";
					$DB['passworddb']="DickSux5841";
					$DB['dbName']="dbs8470793";
					$DB['dbNames']=array('dbs84707932','audevorg_bubblecmslite2','audevorg_bubblecmslite2','audevorg_bubblecmslite2');
					$server_login["db-ionos.php"]=array('server_tag'=>$DB['server_tag'],'server_desc'=>$DB['server_desc'],'current_dir'=>$DB['current_dir'],'server_number'=>$DB['server_number'],'hostname'=>$DB['hostname'],'usernamedb'=>$DB['usernamedb'],'passworddb'=>$DB['passworddb'],'dbName'=>$DB['dbName'],'dbNames'=>$DB['dbNames']);
					//print_r($server_login);
				...
				case '/var/www/html/bcms':	
					...
					$DB['current_dir']="/var/www/html";
					$DB['server_number']=4;
					$DB['hostname']="lin-8133-5299-mysql-primary.servers.linodedb.net";
