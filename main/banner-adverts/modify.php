<?php
	...
	if($_POST['Delete']=="Delete"){
		if(is_array($_POST['DFiles'])){
			...
			$m->WhatToChange("Deleted","Y",false);
			$m->WhatToChange("DeletedDate","NOW()",true);
			$m->AddTable("Adverts");
			$Errors=$m->DoChange();
			
			if($Errors==""){
				$Message="Adverts Deleted";
			}else{
				$Message=$Errors;
			};
		}else{
			$Message="No Adverts Selected To Delete";
		};
	};
	if($_POST['ChannelsID']) $ChannelsID=$_POST['ChannelsID'];
      ...
      <table width="650" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td><form action="modify.php"  method="post" name="form2" >
              <span class="pageheading">Modify / Delete Banners </span><span class="RedText"><?php print $Message; ?></span><br>
              <br>
              Filter Banner Channel
              <select name="ChannelsID" id="ChannelsID" onChange="this.form.submit();">
                <option value="All" selected>- Show All Channels</option>
                <?php
	  	$rslt=$r->RawQuery("SELECT id,Name,Width,Height FROM Channels");
		while($myrow=$r->Fetch_Array($rslt)){
			$tmp=($ChannelsID==$myrow[0] ? "selected" :"");
			echo"<option value='$myrow[0]' $tmp>$myrow[1] - $myrow[2]x$myrow[3]</option>";
		}
	  ?>
              </select>
<br>
              ...
              <table width="100%" border="0" cellpadding="3" cellspacing="1" id="table">
                <tr bgcolor="#363E57" id="tablecell">
                  <td width="23%" class="tabletitle"> User</td>
                  <td class="tabletitle">Banner Name</td>
                  <td width="8%" align="center" class="tabletitle">Modify</td>
                  <td width="9%" align="center" class="tabletitle">Delete</td>
                </tr>
                <?php
					  	$Count=0;
						if((is_numeric($ChannelsID))&&($ChannelsID>0)) $Search="AND ChannelsID=$ChannelsID";
						else $Search="";
					 	$r=new ReturnRecord();
						$sq2=$r->rawQuery("SELECT id,usersID,Name FROM Adverts WHERE Deleted='N' $Search");  
            ...
            <strong><br>
              To Delete an Banner:</strong> select the checkbox for that Banner and then choose Delete button. <br>
            <strong>Tip:</strong> You can select multiple Banners. <br>
          </form></td>
        </tr>
      </table></td>
  </tr>
